...
def get_top_candidates(latent_activations, target_position, num_tokens_per_image = 197):
    ...
    mask = torch.arange(num_tokens, device = latent_activations.device) % 197 == target_position # this mask will allow us to select tokens at the target position
    ...
    top_candidates = torch.topk(selectivity_score, k=20)
