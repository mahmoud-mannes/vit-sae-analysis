...
def evaluate_probe(
    ...
    """Single held-out eval. activations: (N, T, D)."""
    probe.eval()
    _, T, _ = activations.shape
    DL = DataLoader(Data(activations), batch_size=batch_size, shuffle=False, num_workers=0)
    loss_running, acc_running, n = 0.0, 0.0, 0
    with torch.inference_mode():
        for batch in DL:
            batch = batch.float().to(device)
            logits = probe(batch)  # (B, T, P)
            targets = torch.arange(T, device=device).expand(batch.shape[0], -1)
            preds = logits.argmax(dim=-1)
            acc_running += (preds == targets).sum().item() / (batch.shape[0] * T)
            loss_running += F.cross_entropy(
                logits.reshape(-1, logits.shape[-1]),
                targets.reshape(-1),
            ).item()
            n += 1
    return loss_running / max(n, 1), acc_running / max(n, 1)
...
def train_probe_chunk(
    ...
    """One train-only epoch over a specific chunk. Returns the partially trained probe."""
    probe.train()
    _, T, _ = activations.shape
    DL = DataLoader(Data(activations), batch_size=batch_size, shuffle=True, num_workers=0)
    for batch in DL:
        optimizer.zero_grad()
        batch = batch.float().to(device)
        logits = probe(batch)
...
def train_probe_streaming(
    model, processor, source, dataset,
    layer: int,
    ...
    """
    assert probe_type in ["linear", "nonlinear"]
    assert num_images_per_chunk <= threshhold_number_images
    history = {"loss": [], "accuracy": []}

    # probe dims from 5 images
    acts0 = activation_extraction(model, processor, source, layer=layer,
                                  number_images=5, RPI=False, dataset=dataset)
    acts0 = acts0.view(5, -1, acts0.shape[-1]).contiguous()
    _, T, D = acts0.shape
    probe = (LinearProbe(D, T) if probe_type == "linear" else NonLinearProbe(D, T)).to(device)
    optimizer = torch.optim.AdamW(probe.parameters(), lr=lr, weight_decay=weight_decay)

    # fixed held-out val, extracted once
    n_val_images = max(1, int(threshhold_number_images * val_fraction))
    n_train_images = threshhold_number_images - n_val_images
    val_acts = activation_extraction(model, processor, source, layer=layer,
                                     number_images=n_val_images, RPI=False,
                                     shuffle=False, dataset=dataset)
    val_acts = val_acts.view(n_val_images, -1, D).cpu()

    num_chunks = math.floor(n_train_images / num_images_per_chunk)
    for _ in range(num_passes):
        for _ in range(num_chunks):
            acts = activation_extraction(model, processor, source, layer=layer,
                                         number_images=num_images_per_chunk,
                                         RPI=False, shuffle=True, dataset=dataset)
            acts = acts.view(num_images_per_chunk, -1, D).cpu()
            probe = train_probe_chunk(probe, optimizer, acts, batch_size, device)
